//https://ide.geeksforgeeks.org/iD9gf1qZBS
...
//O(n^2)
Node *removedupsNormal(Node *head){
...
//O(n)
//this fails for testcase 1 2 3 4 4 3 : it checks removes only consecutive dups
...
Node *removedupsSet(Node *head){
    ...
    while(current != NULL){
        if(count_int.find(current->data) != count_int.end()){
            Node *temp = current;
            if(current->next != NULL){
                current = current->next;
                prev->next = current;
